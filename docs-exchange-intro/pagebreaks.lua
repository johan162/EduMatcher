...
--
-- Exchange Intro PDF builds concatenate many markdown files into one
-- Pandoc document. Links like `040-topic.md#section-anchor` are therefore
-- intra-document links, but Pandoc treats them as file links by default.
-- For LaTeX output we rewrite these to `#section-anchor` when the target
-- anchor exists in the current document.
--
...
local function resolve_existing_markdown(path)
  ...
  local candidates = {
    -- Paths that work when pandoc is run from docs-exchange-intro/.
    ...
    -- Fallbacks when cwd is nested build directories.
    join_paths("../src", clean),
    join_paths("../docs-exchange-intro", clean),
    join_paths("../docs-exchange-intro/src", clean),
  }
...
local function make_link_filter(known_anchors, h1_ids_by_title)
  return function(el)
    ...
    if not fragment then
      ...
      -- If merged-doc IDs were suffixed for uniqueness (e.g. "chapter-1"),
