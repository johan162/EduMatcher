...
def build_default_engine_field_comment_lines(config: dict[str, object]) -> list[str]:
    ...
    lines.extend(
        [
            ...
            "levels.<NAME>.order_limits:",
            "  Pre-trade order-size and notional caps for this profile.",
            "levels.<NAME>.order_limits.max_order_qty: null",
            "  Largest quantity a single order may carry. Omit for no cap.",
            "levels.<NAME>.order_limits.max_order_value: null",
            "  Largest notional (quantity x price) a single order may carry.",
            "  Omit for no cap. Not checked for MARKET/IOC orders, which carry",
            "  no price. Each cap merges independently:",
            "  symbols.<SYM>.order_limits.<KEY> > symbols.<SYM>.level.",
            "  An unset cap is not enforced -- there is no built-in fallback.",
    ...
    lines.extend(
        [
            ...
            "order_limits:",
            "  Symbol-specific order-size / notional cap override, merged per key",
            "  over the caps of this symbol's risk level.",
