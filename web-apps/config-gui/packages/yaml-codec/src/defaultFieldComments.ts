...
export function buildDefaultEngineFieldCommentLines(): string[] {
  return [
    ...
    "  # weekend:",
    "  #   ... (same five keys; defaults to CLOSED if omitted)",
    "  # holidays:",
    "  #   ... (same five keys; defaults to CLOSED if omitted)",
    "",
    "=============================================================================",
    ...
    "reopening:",
    "  Automated Corridor Expansion. A halt reopens only if the indicative",
    "  uncross price falls inside a corridor centred on the reference price;",
    "  otherwise the corridor widens and another call phase begins.",
    "reopening.enabled: true",
    "  Apply ACE. When false a halt reopens at the equilibrium, uncollared.",
    "reopening.initial_band_pct: 0.10",
    "  Corridor half-width at the start of the first call phase.",
    "reopening.expansions[].widen_pct: 0.10",
    "  Added to the half-width each extension. Additive on the reference",
    "  price, not compounding. The last rung repeats indefinitely, which is",
    "  why there is no maximum-extensions setting.",
    "reopening.expansions[].min_duration_ns: 120000000000",
    "  Minimum length of that extension's call phase.",
    "reopening.random_end_max_ns: 30000000000",
    "  Uniform random tail added to every call phase so the uncross instant",
    "  cannot be targeted. 0 makes reopen times exactly predictable.",
    "reopening.random_seed: null",
    "  Engine-wide; only valid here, never per symbol. null = OS entropy.",
    "",
    ...
    "  Per-symbol ACE switch, overriding circuit_breaker_defaults.",
    "circuit_breaker.reopening.initial_band_pct: 0.10",
    "  Per-symbol reopening corridor half-width.",
    "circuit_breaker.reopening.random_end_max_ns: 30000000000",
    "  Per-symbol random-end bound. The expansion ladder is exchange-level.",
    ...
    "order_retention_sec: 3600",
    "  Seconds a terminal order (FILLED/CANCELLED/EXPIRED/REJECTED) stays in",
    "  the gateway's in-memory cache, which backs GET /orders and the admin",
    "  order table. Resting orders are never evicted. 0 disables eviction.",
    "credentials:",
    ...
    "  weekdays is shorthand for Mon-Fri; an individual day overrides it for just",
    "  that day. A day with no entry (from either) is CLOSED.",
    "weekend / sat, sun",
    "  weekend is shorthand for Sat+Sun; mutually exclusive with specifying sat",
    "  and/or sun individually. Defaults to CLOSED like any other day.",
    "holidays",
    "  Applied instead of the weekday's own entry on a bank holiday (per the",
    "  country field). Defaults to CLOSED, same as an ordinary unset day.",
    "",
    "Each block above (weekdays/mon/tue/wed/thu/fri/weekend/sat/sun/holidays)",
    "takes the same five keys, all required whenever the block is present:",
    "pre_open: 09:00",
