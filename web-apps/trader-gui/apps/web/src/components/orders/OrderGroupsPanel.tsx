...
export function OrderGroupsPanel({ orders, onCancelGroup }: OrderGroupsPanelProps) {
  ...
  return (
    <section aria-label="Order groups" className="flex flex-col gap-1">
      <h2 className="text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Groups</h2>
        ...
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {groups.map((g) => {
              ...
              return (
                <tr key={`${g.kind}:${g.id}`} className={`border-b border-raised ${accent}`}>
                  ...
                  <td className="px-2 py-1 font-mono text-fg">{g.id}</td>
                  <td className="px-2 py-1 font-mono text-fg-dim">{symbols}</td>
