...
function stateClasses(state: string): string {
  ...
  return "bg-slate-700 text-slate-200"; // CANCELLED / MISSING / unknown
}
  ...
  const legRow = (
    ...
    return (
      <div className="flex flex-col gap-0.5">
        ...
        <span className="text-[9px] text-fg-faint">{status}</span>
      </div>
    );
  };

  return (
    <div className="flex flex-col gap-2 rounded border border-line bg-deep p-3">
        ...
        <div className="flex gap-1.5">
          ...
          {quote && (
            <button
              ...
              className="rounded border border-line px-2 py-0.5 text-[11px] text-fg-dim hover:text-ask"
            >
              Cancel
            </button>
          )}
      ...
      {quote ? (
        <>
          ...
          <div className="text-[10px] text-fg-faint">Quote ID: {quote.quote_id}</div>
        </>
      ) : (
        !formOpen && <p className="text-[11px] text-fg-faint">No active quote.</p>
      )}
