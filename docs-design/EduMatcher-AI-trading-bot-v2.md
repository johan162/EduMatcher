...
```
LiveOrder:
  order_id: str        # assigned by engine; present after ack
...
**Startup / reconnect seeding.** On connect, after `system.gateway_auth` is
received, the bot sends `system.halt_status_request` (with its `gateway_id`).
The engine replies on `system.halt_status.<GW_ID>` with the list of currently
halted symbols. The bot pre-populates its per-symbol `halted` set from this
reply before beginning to trade, so it never submits into a symbol that was
...
| `order.fill.<GW_ID>` | Fill event with `order_id`, `client_tag`, `fill_qty`, `price` |
...
| `system.halt_status.<GW_ID>` | Reply to halt-status request on connect; list of currently halted symbols with level and resumption mode |
...
| `system.halt_status_request` | Immediately after `system.gateway_auth` to seed per-symbol halt state (§9.3) |
| `order.new` | Decision Engine emits `order_type=LIMIT`, `tif ∈ {DAY, ATO, ATC}`, `client_tag` (bot-generated UUID), optional `smp` (§6.3) |
...
| 22.5 | Position snapshot request | Risk resync on restart (§13) | Medium | Proposal |
