...
class TestCountryReachesTheEngineConfig:
    """`country:` is documented, generated by pm-config-gen and checked by
    pm-cverifier — but nothing ever put it in EngineConfig, so the compiled
    artifact dropped it and pm-scheduler had to re-parse the YAML to find it.
    """

    def test_country_is_loaded(self, tmp_path) -> None:
        cfg = load_engine_config(
            _write_yaml(
                tmp_path,
                """
    ...
    def test_country_defaults_when_absent(self, tmp_path) -> None:
        cfg = load_engine_config(
            _write_yaml(
                tmp_path,
                """
        ...
        assert cfg.country == "Sweden"

    def test_a_blank_country_falls_back_rather_than_being_stored(
        self, tmp_path
    ) -> None:
        cfg = load_engine_config(
            _write_yaml(
...
class TestScheduleTimesAreCanonical:
    def test_an_unquoted_time_is_recovered_not_stored_as_minutes(
        self, tmp_path
    ) -> None:
        # PyYAML reads an unquoted `9:30` as the sexagesimal integer 570. The
        # loader used to str() it straight into the config, so the engine held
        # "570" while pm-scheduler, which normalised, held "09:30" — the same
        # file, two answers.
        cfg = load_engine_config(
            _write_yaml(
                tmp_path,
