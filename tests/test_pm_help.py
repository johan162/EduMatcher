...
def test_every_command_has_required_fields() -> None:
    for cmd in ALL_COMMANDS:
        ...
        assert (
            cmd.category in CATEGORY_ORDER
        ), f"{cmd.name} has unknown category {cmd.category!r}"
...
def test_every_related_command_exists() -> None:
    for cmd in ALL_COMMANDS:
        for related in cmd.related:
            assert (
                get_command(related) is not None
            ), f"{cmd.name} references unknown related command {related!r}"
...
def test_index_mode_prints_version_and_data_dir(
...
def test_index_mode_table_format_uses_box_drawing(
    capsys: pytest.CaptureFixture[str],
) -> None:
    main(["--format", "table"])
    out = capsys.readouterr().out
...
def test_index_mode_text_format_has_no_box_drawing(
    capsys: pytest.CaptureFixture[str],
) -> None:
    main(["--format", "text"])
    out = capsys.readouterr().out
...
def test_index_mode_categories_appear_as_section_titles(
    capsys: pytest.CaptureFixture[str],
) -> None:
    main(["--format", "text"])
    out = capsys.readouterr().out
...
def test_man_page_accepts_bare_name_without_pm_prefix(
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_man_page_shows_subcommands_when_present(
    capsys: pytest.CaptureFixture[str],
) -> None:
    main(["pm-opctl-cli"])
    out = capsys.readouterr().out
...
def test_man_page_every_command_renders_without_error(
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_unknown_command_exits_nonzero_with_suggestion(
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_prog_name_follows_argv0(
...
def test_prog_name_defaults_to_pm_help(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
