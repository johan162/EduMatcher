...
[`engine_config.yaml`](010-configuration.md) file interactively.

It is a **companion to**, not a replacement for, the
[`pm-config-gen`](010-configuration.md#generate-configs-with-pm-config-gen) CLI.
...
| **`pm-cverifier` on `PATH`** | Provided by the Poetry env, or set `CVERIFIER_COMMAND="poetry run pm-cverifier"` so the backend can locate it. See [Config Verifier](020-config-verifier.md). |
...
- **Gateways** are the participant sessions permitted to connect, each with a
  **role** — `TRADER`, `MARKET_MAKER`, or `ADMIN`. Roles determine which
  ...
  [ALF gateway](220-alf-gateway.md) chapter. A `MARKET_MAKER` gateway unlocks the
  Market Maker tab and quote seeding; an `ADMIN` gateway can issue exchange-wide
  halt/resume commands (see
  [ADMIN-role operator controls](120-risk-controls.md#admin-role-operator-controls)).
...
[initial public offering](120-risk-controls.md#day-one-ipo-behaviour): on the
...
[reference prices](120-risk-controls.md#rolling-reference-window) have nothing to
...
[Collar reference price selection](010-configuration.md#collar-reference-price-selection).
...
described in [Price collars](120-risk-controls.md#price-collars); order-level
behaviour is covered in [Order Types](060-order-types.md).
...
- Define **named risk levels** (e.g. `CORE`, `HIGH_BETA`) that symbols reference
  by name — see
  [Global level profiles](120-risk-controls.md#global-level-profiles-l1l2l3-style)
  and [per-symbol risk-level assignment](010-configuration.md#per-symbol-risk-level-assignment).
...
[Circuit breakers](120-risk-controls.md#circuit-breakers).

The **Circuit Breakers** tab (Intermediate+) presents the ladder as a table
(Level, Shift %, Halt, Rest of day, Resumption) plus the
[reference window](120-risk-controls.md#rolling-reference-window) and the global
...
[Market Making](090-market-maker.md); obligation enforcement is detailed under
[MM obligations enforcement](090-market-maker.md#mm-obligations-enforcement).
...
- **Max spread (ticks)** — the widest allowed bid–ask spread; wider quotes are
  rejected ([spread constraint](090-market-maker.md#spread-constraint)).
- **Min quantity** — the minimum displayed size on each side
  ([minimum size constraint](090-market-maker.md#minimum-size-constraint)).

This layer maps to `mm_obligation_defaults` in the YAML
([reference](010-configuration.md#market-maker-obligation-defaults)).
...
allowed spread. See [per-symbol overrides](090-market-maker.md#per-symbol-overrides).
...
[**quote refresh policy**](090-market-maker.md#quote-refresh-policy) — when seeded
...
[startup seeding](090-market-maker.md#startup-seeding-pre-loading-quotes-from-config)).
...
See [controlling when seeds apply](090-market-maker.md#controlling-when-seeds-are-applied-seed_once)
and [choosing TIF for seeds](090-market-maker.md#choosing-tif-for-seed-quotes).
...
the backend, which shells out to [`pm-cverifier`](020-config-verifier.md) and
