...
| `GET /reference/schedule` | `{ "sessions_enabled":..., "country"?:..., "schedule": {mon, tue, wed, thu, fri, sat, sun, holidays: {pre_open, opening_auction_start, continuous_start, closing_auction_start, closing_auction_end} \| null, today: {...} \| null, today_is_holiday: bool} \| null, "config_version":... }` | The resolved weekly table is nested under `schedule`, which is the same record `system.session_schedule` carries; each day (or `holidays`) is `null` for CLOSED. `today`/`today_is_holiday` are the engine's own resolution of which entry applies right now, so a client never needs its own holiday calendar. `schedule: null` means no `schedule:` block is configured |
