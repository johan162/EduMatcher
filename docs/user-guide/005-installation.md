...
```bash
curl -fsSL .../install.sh | bash -s -- --config ten-nominal --version 0.40.5rc1
```
...
curl -fsSL https://raw.githubusercontent.com/johan162/EduMatcher/main/deployment/vm/curl_setup_vm.sh | \
    bash -s -- --version 0.40.5rc1 --snapshot
...
bash curl_setup_vm.sh --version 0.40.5rc1 --snapshot
```
...
Every `pm-*` command completes its subcommands, options and `choices=`
values in bash and zsh — `pm-audit-cli ev<Tab>` completes to `events`,
`pm-engine --log-level <Tab>` lists `CRITICAL ERROR WARNING INFO DEBUG`, and
so on. `pm-help --completion bash|zsh` prints the script; nothing needs to be
generated by hand.

**Container**: nothing to do — the backend image builds completion in at
image-build time and every interactive `make shell` / `make ssh` session
already has it.

**pipx or `pip install` into a venv**: add one line to your shell profile —
...
(( $+commands[pm-help] )) && eval "$($commands[pm-help] --completion zsh)"
```

**Poetry checkout**: the venv is usually not active when your shell profile
runs, so point at it by its absolute path —

```bash
# ~/.bashrc
EM=/path/to/EduMatcher/.venv/bin/pm-help
...
An editable install (`poetry install`) reads the completion scripts straight
from the checkout, so they follow whichever branch is checked out. Calling
`pm-help` at every shell start (rather than sourcing a saved copy) keeps
completion matched to the installed version, at the cost of `pm-help`'s
own ~0.2s start-up per new shell. To avoid that, cache it once per upgrade
instead:

```bash
pm-help --completion bash > ~/.local/share/edumatcher/pm-completion.bash
# ~/.bashrc:  source ~/.local/share/edumatcher/pm-completion.bash
```

```zsh
pm-help --completion zsh > ~/.local/share/edumatcher/pm-completion.zsh
# ~/.zshrc:   eval "$(<~/.local/share/edumatcher/pm-completion.zsh)"
```

zsh's own completion system (`compinit`) must already be initialised — the
snippets above assume the usual `autoload -Uz compinit && compinit` earlier
