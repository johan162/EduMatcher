...
```bash
curl -fsSL https://raw.githubusercontent.com/johan162/EduMatcher/main/vm/curl_setup_vm.sh | bash -s -- --version 0.20.0 --snapshot
```
...
```
./vm/build_multipass_vm.sh --version 0.20.0
```
...
```
./vm/build_multipass_vm.sh --name edumatcher-072 --version 0.20.0 --snapshot
```
...
multipass exec edumatcher-071 -- sudo /tmp/install_edumatcher_runtime.sh --version 0.20.0
```
