...
def _levels_equal(
    ...
    if len(paper) != len(engine):
        ...
        # Report extra levels
        for extra in (paper if len(paper) > len(engine) else engine)[
            min(len(paper), len(engine)) :
        ]:
            source = "paper" if len(paper) > len(engine) else "engine"
            diffs.append(
                f"{label}[extra {source}] price={extra['price']} qty={extra['qty']}"
            )
        return diffs
...
def _compare_level(
    ...
    if abs(p["price"] - e["price"]) > 1e-9:
        diffs.append(f"{label}[{idx}] price: paper={p['price']}, engine={e['price']}")
...
def main() -> None:
    ...
    ap.add_argument(
        "--paper",
        default=str(PAPER_RESULT),
        metavar="FILE",
        help="Paper-trade result JSON",
    )
    ap.add_argument(
        "--engine",
        default=str(ENGINE_RESULT),
        metavar="FILE",
        help="Engine result JSON",
    )
    ap.add_argument(
        "--tolerance",
        type=float,
        default=0.0,
        metavar="FRAC",
        help="Relative qty tolerance (default: 0 = exact match)",
    )
    ap.add_argument(
        "--symbols",
        nargs="+",
        default=None,
        metavar="SYM",
        help="Restrict comparison to these symbols",
    )
    args = ap.parse_args()
